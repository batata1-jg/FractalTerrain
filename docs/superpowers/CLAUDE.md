...
| `plans/`  | `2026-08-19-river-primitive-grid-lut.md` — task-by-task plan for the carve rewrite (**checkboxes are stale**: the rewrite landed, but no task was ticked); `2026-08-24-heightmap-slice-sampling.md` — task-by-task plan for the per-chunk tensor slice fill and the relief/biome cache budgets | Tracing which step of the carve rewrite introduced a given behaviour, or executing the slice-sampling change |
