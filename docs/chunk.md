...
> **Revision:** 3582335 · **Retrieved:** 2026-07-28  
...
- **1.20.5** — Added `/gamerule spawnChunkRadius`. The default spawn chunk radius has been decreased to 2 (from 10).
- **1.21.9** — Removed the aforementioned game rule, along with spawn chunks altogether.
- **1.21.9** — A dimension that has any player activity, forceloaded chunks, active portals, or ender pearls in flight will be considered "active" and will keep processing chunks and entities.
- **1.21.9** — The game will ensure that the following chunks are fully loaded before the player joins or the server starts, in the same way as spawn chunks formerly were: chunks loaded by the `/forceload` command, chunks active due to portal activity, thrown ender pearls (Singleplayer only), the small area of chunks around the player (Singleplayer only), chunks required to assign the global world spawn (World c
- **1.21.11** — Chunks now fade in instead of appearing out of nowhere, with the fade-in time adjustable through Video Settings.
- **1.21.11** — Legacy Console Edition
...
Chunks generate around players when they first enter the world. In *Java Edition*, the generating and loading progress are displayed on the loading world screen. As players wander around the world, new chunks generate as needed.
...
Since *Minecraft* worlds are 30 million blocks in each [cardinal direction](https://en.wikipedia.org/wiki/Cardinal_direction)‌[*Java Edition only*] and contain an extreme number of chunks, the game loads only certain chunks in order to make the game playable. Unloaded chunks are unprocessed by the game and do not process any of the game aspects.
...
|  | Inaccessible | 34 and above | No game aspect is available or accessible, but world generation occurs here. |
...
The specific instructions are: *Update start ticket image to 1.20.5*
...
Each dimension has its own **idle timeout**. It increases by 1 every game tick, while the game is not in `/tick freeze` mode. If there are chunk loading tickets in the dimension that keep the dimension active (i.e. any player, force-loaded chunks, active portal tickets, or ender pearls) or an entity enters the dimension via portal travel, the timeout is reset to 0.
...
|  |  |  |
| --- | --- | --- |
| Block Pos | (0, 0, 0) | [0 0 0] |
| Chunk Pos | (0, 0, 0) | [0 0 in r.0.0.mca] |
...
This section is missing information about: *Bedrock Edition* history

Please expand the section to include this information. Further details may exist on the [talk page](https://minecraft.wiki/w/Talk:Chunk).

### *Java Edition*

| Java Edition pre-Classic | | | | | | | |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Cave game tech test | | | | | | | The precursors to chunks are showcased. |
| Java Edition Infdev | | | | | | | |
| 20100227-1414 | | | | | | | Map height is not custom anymore. The world height has been changed to 128. |
| 20100327 | | | | | | | The game now saves the map in chunks, as part of the transition from Indev to Alpha file format. |
| *Java Edition* | | | | | | | |
| 1.2.1 | | | 12w07a | | | | The chunk height has been increased to 256, as part of the transition from Region to Anvil file format. |
| 1.3.1 | | | 12w22a | | | | Pressing `F` reloads the chunks. |
| 12w23a | | | | Pressing `F` no longer reloads chunks. |
...
| 21w06a | | | | Maximum buiding height in Overworld has been increased to 319 and minimum height has been dropped to -64, making chunks 384 blocks high in total. |
| 21w15a | | | | The chunk height changes have been reverted. |
| 1.18 | | | Experimental Snapshot 1 | | | | Chunks in Overworld are again 384 blocks high. |
| 21w37a | | | | Render distance now causes chunks to load in a cylinder shape around the player instead of a square. |
| 21w38a | | | | Added simulation distance, which affects entity ticking. |
| 1.20.5 | | | 24w03a | | | | Added `/gamerule spawnChunkRadius`. The default spawn chunk radius has been decreased to 2 (from 10). |
| 1.21.9 | | | 25w31a | | | | Removed the aforementioned game rule, along with spawn chunks altogether. |
...
| The game will ensure that the following chunks are fully loaded before the player joins or the server starts, in the same way as spawn chunks formerly were: chunks loaded by the `/forceload` command, chunks active due to portal activity, thrown ender pearls (Singleplayer only), the small area of chunks around the player (Singleplayer only), chunks required to assign the global world spawn (World creation only). |
| 1.21.11 | | | 25w44a | | | | Chunks now fade in instead of appearing out of nowhere, with the fade-in time adjustable through Video Settings. |
...
| TU1 | CU1 | 1.00 | 1.00 | 1.00 | Patch 1 | 1.0.1 | Added chunks. |
| TU12 | The chunk height has been increased to 256. |

## Issues

Issues relating to "Chunk" are maintained on the bug tracker. Issues should be reported and viewed [there](https://bugs.mojang.com/issues/?jql=project%20in%20%28MC%2C%20MCPE%29%20AND%20%28resolution%20is%20EMPTY%20OR%20resolution%20in%20%281%2C%202%2C%206%29%29%20AND%20%28summary%20~%20%22Chunk%22%29%20ORDER%20BY%20resolution%20DESC).
