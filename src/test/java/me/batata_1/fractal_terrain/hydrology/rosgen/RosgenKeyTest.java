...
class RosgenKeyTest {
    ...
    void steepestSlopeGivesAaRegardlessOfEverythingElse() {
        ...
        assertEquals(River.RosgenType.Aa, RosgenKey.classify(with(baseC(), 0.20, 8.0, 30.0, null, null)));
    }

    @Test
    void steepSlopeGivesA() {
        assertEquals(River.RosgenType.A, RosgenKey.classify(with(baseC(), 0.06, 8.0, 30.0, null, null)));
    }

    @Test
    void entrenchedAndNarrowGivesG() {
        assertEquals(River.RosgenType.G, RosgenKey.classify(with(baseC(), 0.03, 1.2, 8.0, null, null)));
    }

    @Test
    void entrenchedAndWideGivesF() {
        assertEquals(River.RosgenType.F, RosgenKey.classify(with(baseC(), 0.03, 1.2, 20.0, null, null)));
    }

    @Test
    void moderatelyEntrenchedGivesB() {
        assertEquals(River.RosgenType.B, RosgenKey.classify(with(baseC(), 0.03, 1.8, 20.0, null, null)));
    }
    ...
    void bAndGShareASlopeBandAndAreSeparatedByEntrenchmentOnly() {
        ...
        assertEquals(River.RosgenType.G, RosgenKey.classify(g));
        assertEquals(River.RosgenType.B, RosgenKey.classify(b));
    }

    @Test
    void nearBaseLevelFlatAndVeryWideFloodProneGivesDA() {
        assertEquals(River.RosgenType.DA, RosgenKey.classify(with(baseC(), 0.001, 6.0, 20.0, 10.0, 2.0)));
    }
    ...
    void daIsTestedBeforeDSoBraidingNeverStealsAnAnastomosingReach() {
        ...
        assertEquals(River.RosgenType.DA, RosgenKey.classify(both));
    }
    ...
    void wideChannelAboveTheBraidThresholdGivesD() {
        ...
        assertEquals(River.RosgenType.D, RosgenKey.classify(new ReachMetrics(slope, 6.0, 20.0, width, 80.0)));
    }
    ...
    void narrowChannelAboveTheBraidThresholdIsNotD() {
        ...
        final double slope = RosgenKey.braidThreshold(width) * 2.0;
        assertEquals(River.RosgenType.E, RosgenKey.classify(new ReachMetrics(slope, 6.0, 8.0, width, 80.0)));
    }

    @Test
    void slightlyEntrenchedAndNarrowGivesE() {
        assertEquals(River.RosgenType.E, RosgenKey.classify(new ReachMetrics(0.001, 6.0, 8.0, 2.0, 80.0)));
    }

    @Test
    void slightlyEntrenchedAndWideGivesC() {
        assertEquals(River.RosgenType.C, RosgenKey.classify(new ReachMetrics(0.001, 6.0, 20.0, 2.0, 80.0)));
    }
    ...
    void deadBandKeepsTheUpstreamTypeWhenEntrenchmentSitsOnAThreshold() {
        ...
        assertEquals(River.RosgenType.C, RosgenKey.classify(onBoundary));
        assertEquals(River.RosgenType.B, RosgenKey.applyDeadBand(onBoundary, River.RosgenType.C, River.RosgenType.B));
    }
    ...
    void deadBandKeepsTheUpstreamTypeWhenWidthDepthSitsOnAThreshold() {
        ...
        assertEquals(River.RosgenType.C, RosgenKey.classify(onBoundary));
        assertEquals(River.RosgenType.E, RosgenKey.applyDeadBand(onBoundary, River.RosgenType.C, River.RosgenType.E));
    }

    @Test
    void deadBandDoesNotSuppressAChangeFarFromAnyThreshold() {
        final ReachMetrics clear = new ReachMetrics(0.001, 6.0, 20.0, 2.0, 80.0);
        assertEquals(River.RosgenType.C, RosgenKey.applyDeadBand(clear, River.RosgenType.C, River.RosgenType.E));
    }

    @Test
    void deadBandPassesThroughWhenThereIsNoUpstreamNeighbour() {
        final ReachMetrics onBoundary = new ReachMetrics(0.001, 2.25, 20.0, 2.0, 80.0);
        assertEquals(River.RosgenType.C, RosgenKey.applyDeadBand(onBoundary, River.RosgenType.C, null));
    }
}
