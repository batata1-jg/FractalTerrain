...
public class FractalTerrainInstance {
    ...
    public static synchronized void setServer(MinecraftServer server) {
        if(FractalTerrainDensityFunctionTypes.InterpolatedFromMap.isActive()){
            if(curServer != null || pathMundo != null || ENV != null || post != null) throw new IllegalStateException("aready initialized");
            curServer = server;
            ...
            try(OrtEnvironment.ThreadingOptions opts = new OrtEnvironment.ThreadingOptions() ) {
                opts.setGlobalInterOpNumThreads((Runtime.getRuntime().availableProcessors()>>2) == 0 ? 1 :( Runtime.getRuntime().availableProcessors()>>2));
            ...
            long seed = FractalTerrainInstance.getServer().getSaveProperties().getGeneratorOptions().getSeed();
    ...
    public static MinecraftServer getServer() { return curServer;}

    public static Path getDir() { return Path.of(pathMundo + "/fractal_terrain"); }
