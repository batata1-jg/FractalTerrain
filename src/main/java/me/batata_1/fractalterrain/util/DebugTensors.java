...
public class DebugTensors {
    ...
    public static void seeTensor(OnnxTensor op , String name, boolean seeAvg , int channel ) throws IOException {
        ...
        if(op.getInfo().getShape().length > 3) {
            ...
            tl = new Tile(op.getFloatBuffer().array(),new long[]{1,op.getInfo().getShape()[0],op.getInfo().getShape()[1]});
        }
...
//                LOGGER.info("oche?????{} {}", i, j);
                ...
                ftu[(int) (j + tl.getShape()[1] * i)] = tl.entryAt(new long[]{channel, i, j})
                        / tl.entryAt(new long[]{(tl.getShape()[0] - 1), i, j});
            }
        Tile t = new Tile(ftu,new long[]{tl.getShape()[1],tl.getShape()[2]});

        Path path = FractalTerrainInstance.getServer().getSavePath(WorldSavePath.ROOT).normalize();
...
//                LOGGER.info("osamaaaaaaa {}",vi);
                int v = (int) (255F * vi);

                arr[(int) (j + i*t.getShape()[0])] = v;
            }
        }
        BufferedImage outputImage = new BufferedImage((int) t.getShape()[0], (int) t.getShape()[1], BufferedImage.TYPE_BYTE_GRAY);
