...
public class DebugTensors {
  ...
  public static void seeTensor(OnnxTensor op, String name, boolean seeAvg, int channel)
      throws IOException {
    ...
    for (int i = 0; i < tl.getShape()[1]; i++)
      for (int j = 0; j < tl.getShape()[2]; j++) {
        ...
        ftu[(int) (j + tl.getShape()[1] * i)] =
            tl.entryAt(new long[] {channel, i, j})
    ...
    BufferedImage outputImage =
        new BufferedImage(
            (int) t.getShape()[0], (int) t.getShape()[1], BufferedImage.TYPE_BYTE_GRAY);
