...
public class FractalTerrainBiomeSource extends BiomeSource {
  ...
  public final Either<
          MultiNoiseUtil.Entries<RegistryEntry<Biome>>,
  ...
  static {
    ...
    CODEC =
        Codec.mapEither(CUSTOM_CODEC, PRESET_CODEC)
            .xmap(
  ...
  public static FractalTerrainBiomeSource create(
      MultiNoiseUtil.Entries<RegistryEntry<Biome>> biomeEntries) {
  ...
  private MultiNoiseUtil.Entries<RegistryEntry<Biome>> getBiomeEntries() {
    return this.biomeEntries.map(
        (entries) -> entries, (parameterListEntry) -> parameterListEntry.value().getEntries());
  }
