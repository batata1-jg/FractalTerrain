...
public class FractalTerrainBiomeSource extends BiomeSource {
    ...
    public final Either<MultiNoiseUtil.Entries<RegistryEntry<Biome>>, RegistryEntry<MultiNoiseBiomeSourceParameterList>> biomeEntries;
    ...
    static {
        ...
        CODEC = Codec.mapEither(CUSTOM_CODEC,  PRESET_CODEC).xmap(
                FractalTerrainBiomeSource::new,
                (FractalTerrainBiomeSource biomeSource) -> biomeSource.biomeEntries
        ).codec();
    }



    private FractalTerrainBiomeSource(Either<MultiNoiseUtil.Entries<RegistryEntry<Biome>>, RegistryEntry<MultiNoiseBiomeSourceParameterList>> biomeEntries) {
    ...
    public Pair<BlockPos, RegistryEntry<Biome>> locateBiome(int x, int y, int z, int radius, int blockCheckInterval, Predicate<RegistryEntry<Biome>> predicate, Random random, boolean bl, MultiNoiseUtil.MultiNoiseSampler noiseSampler) {
        ...
        for(int i=0 ; i<getBiomeEntries().getEntries().size() ; i++) {
            if(predicate.test(getBiomeEntries().getEntries().get(i).getSecond())) {
                return Pair.of(new BlockPos(0,0,0),getBiomeEntries().getEntries().get(i).getSecond());
            }
        }
