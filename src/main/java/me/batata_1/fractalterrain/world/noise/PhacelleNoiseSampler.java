...
public class PhacelleNoiseSampler extends VoronoiNoiseSampler{
    ...
    public float sample(final Number x , final Number z) {
        try {
            final double[] grads = Gradients.entryGradMagnitude(x.intValue(),z.intValue(),1,reliefSource.get().getStorage());

            return this.sample(x.floatValue(),z.floatValue(),
                    (float) grads[0],
                    (float) grads[1],
                    (float) grads[2]);
    ...
    public float sample(final float x, final float z, final float gradX, final float gradY, final float grad) {
        ...
        for(byte dx = -1; dx <=1 ; dx++) {
            for(byte dz = -1; dz <=1 ; dz++) {
                ...
                val += weight*Math.cos(freq*(-sinTheta*(xi-xCurSquare-dx)+cosTheta*(zi-zCurSquare-dz)));
                avg += weight;
            }
        }
