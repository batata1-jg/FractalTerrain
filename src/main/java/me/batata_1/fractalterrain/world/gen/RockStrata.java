...
public abstract class RockStrata {
    ...
    public record Settings(double layerSpacing, int maxNumLayers, int layerOffset, CompletableFuture<Function<double[],Double>> layer_function ) implements SettingsRegistry.Settings {
    ...
    private static int binarySearchLayer( double x , double z , double y , int p , int q ,Function<double[],Double> f_layer) {
        while( p != q ) {
            int m = -((-p-q) >> 1);
