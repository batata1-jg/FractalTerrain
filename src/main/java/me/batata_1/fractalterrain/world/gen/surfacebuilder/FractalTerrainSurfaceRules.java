...
public class FractalTerrainSurfaceRules {
  ...
  public static MaterialRules.MaterialRule createDefaultRule(
    ...
    MaterialRules.MaterialRule placeSandAndSandstone =
        MaterialRules.sequence(
            MaterialRules.condition(MaterialRules.STONE_DEPTH_CEILING, SANDSTONE), SAND);
    MaterialRules.MaterialRule placeGravelAndStone =
        MaterialRules.sequence(
            MaterialRules.condition(MaterialRules.STONE_DEPTH_CEILING, STONE), GRAVEL);
    ...
    MaterialRules.MaterialRule placePowderSnow1 =
        MaterialRules.condition(
    ...
    MaterialRules.MaterialRule placePowderSnow2 =
        MaterialRules.condition(
        ...
        realizeFrozenPeaks_SnowySlopes_JaggedPeaks_Grove_WindsweptSavanna_WindsweptGravellyHills_OldGrowthPineTaiga_IceSpikes_MangroveSwamp_MushroomFields =
            MaterialRules.sequence(
                ...
                MaterialRules.condition(
                    MaterialRules.biome(BiomeKeys.GROVE),
                    MaterialRules.sequence(
                        placePowderSnow2, MaterialRules.condition(isAboveWaterZero, SNOW_BLOCK))),
                realizeStonyPeaks_StonyShore_WindsweptHills_DripstoneCaves,
                MaterialRules.condition(
                    MaterialRules.biome(BiomeKeys.WINDSWEPT_SAVANNA),
                    MaterialRules.sequence(
    ...
    MaterialRules.MaterialRule realizeEverything =
        MaterialRules.sequence(
            MaterialRules.condition(
                    ...
                    MaterialRules.condition(
                        ...
                        MaterialRules.sequence(
                            MaterialRules.condition(
                                isAboveY63,
                                MaterialRules.condition(
                                    MaterialRules.not(isAbove74WithStoneDepth), ORANGE_TERRACOTTA)),
                            MaterialRules.terracottaBands())),
                    MaterialRules.condition(
          ...
          MaterialRules.condition(
              MaterialRules.not(
                  MaterialRules.verticalGradient(
                      "bedrock_roof", YOffset.belowTop(5), YOffset.getTop())),
              BEDROCK));
    }
    ...
    if (bedrockFloor) {
      builder.add(
          MaterialRules.condition(
              MaterialRules.verticalGradient(
                  "bedrock_floor", YOffset.getBottom(), YOffset.aboveBottom(5)),
              BEDROCK));
    }

    FractalTerrainMaterialRules.SteepSlope fracSteepSlope =
        new FractalTerrainMaterialRules.SteepSlope(1200, 0.2F);
  ...
  public static MaterialRules.MaterialRule createLargeRule(
    ...
    FractalTerrainMaterialRules.SteepSlope fracSteepSlope =
        new FractalTerrainMaterialRules.SteepSlope(1200, 1F);
