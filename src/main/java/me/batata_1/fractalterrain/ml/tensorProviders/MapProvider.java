           ...
           for(int j=0 ; j< shape[of+1] ; j++) {
               for(int k=0; k< shape[of+2] ; k++) {
                   ...
                   double val = switch (i) {
                       ...
                       case 1 -> sampleTemperature(x, z)*tempSettings.amplitude()*Math.max(sampleElev(x,z),0);
                       case 2 -> sampleThermalAmplitude(x, z)*tempSTDSettings.amplitude();
                       case 3 -> samplePrecipitation(x,z)*precipSettings.amplitude();
                       case 4 -> samplePrecipitationAmplitude(x, z)*precipSTDSettings.amplitude()*samplePrecipitation(x,z);
