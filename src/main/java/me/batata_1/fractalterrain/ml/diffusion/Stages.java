...
public class Stages {
    ...
    public static class LatentStage extends StorageInterface {
        ...
        public LatentStage() {
            super( new EntryStorage<>("latent/" + startInstanceNum, TileRegion::new,32),32*32*6,new long[]{6,32,32});
        ...
        @Override
        public OnnxTensor[] runInference(int x, int z) throws OrtException, ExecutionException, InterruptedException, IOException {
            ...
            for(int i=0 ; i<7 ; i++)
                for(int j=xi ; j<xi+4 ; j++)
                    for(int k=zi ; k<zi+4 ; k++)
                        fl.put(
                                coarseModel.getValue(Pair.of(j,k),i)
                        );
            fl.flip();
            OnnxTensor coarse_tensor =OnnxTensor.createTensor(ENV,fl,new long[]{7,4,4});

            OnnxTensor sample;
            if(latentStageInstanceNumber==0)
                sample = sampleAvgNull(new long[]{6,64,64});
            else {
    ...
    //        seeTensor(noise,"noise latent_t" + latentStageInstanceNumber + "__" + xz.getFirst() + " " + xz.getSecond(), false);
    ...
    public static class CoarseStage extends StorageInterface {
        ...
        protected CoarseStage() throws OrtException, IOException {
            ...
            sigmas = new float[]{8.0000e+01F, 5.9658e+01F, 4.3920e+01F, 3.1884e+01F, 2.2794e+01F,
                    ...
                    9.8973e-02F, 4.4883e-02F, 1.8401e-02F, 6.6217e-03F, 2.0000e-03F,
                    0};
        }
        ...
        public OnnxTensor[] runInference(int x , int z) throws OrtException {
            ...
            for( int i=0 ; i<sigmas.length-1 ; i++) {
                ...
                inputs = Map.of(
                        "sample",(OnnxTensor) out.get(0),
                        "prev_sample",(OnnxTensor) out.get(1),
                        "sigma_prev",OnnxTensor.createTensor(ENV,sigmas[i-1]),
                        "sigma_id",OnnxTensor.createTensor(ENV,sigmas[i]),
                        "sigma_post",OnnxTensor.createTensor(ENV,sigmas[i+1]),
                        "cond_img",img
                );
