...
public class Spline {
    ...
    public Spline(float[] xs, float[] ys, float[] ks) {
        if(xs.length != ys.length || xs.length != ks.length ) throw new IllegalArgumentException("spline xs and ys len differ");
        ...
        x = xs;
        y = ys;
        for(int i=0 ; i<numPts-1 ; i++) if( xs[i+1] - xs[i] <= 0 ) throw new IllegalArgumentException("xs not strictly increasing");
