...
public class EntryStorage<T extends Tile> {
    ...
    private synchronized void bootstrap() {
        ...
        if(!file.exists()) if(file.mkdirs())
            LOGGER.info("created tile dir in: {}", getEntryDir());
...
//        LOGGER.info("list os files: {}" , (Object) file.list());
    ...
    public boolean inBorder(Pair<Integer,Integer>xz) {
        ...
        return intra.getFirst() == entry_len-1 || intra.getFirst() == 0 || intra.getSecond() == entry_len-1 || intra.getSecond() == 0;
    }
...
//        LOGGER.info("mas ele vem aqui ne?");
        CompletableFuture<T> ct = t.thenApply(entry -> {
//            LOGGER.info("oieeeeeeeeeee");
            try {
...
//        LOGGER.info("e aqui?");
    ...
    //xz inter coords
    public boolean existsEntry(Pair<Integer,Integer> xz) { return GENERATED_ENTRIES.contains(xz);}
...
//            LOGGER.info("reading_tile");
            CompletableFuture<T> ct = CompletableFuture.supplyAsync(() -> {
                ...
                if( !file.exists() ) {
                    ...
                    LOGGER.error("file {}, aka: {}-{} not exist",file.getAbsolutePath()
                            ,xz.getFirst(),xz.getSecond());
