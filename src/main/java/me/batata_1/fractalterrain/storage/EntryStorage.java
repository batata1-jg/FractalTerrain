...
public class EntryStorage<T extends Tile> {
  ...
  private final ConcurrentHashMap<Pair<Integer, Integer>, CompletableFuture<T>> CACHE =
  ...
  public EntryStorage(
      String path, Supplier<T> s, int entryLen, Function<Pair<Integer, Integer>, T> f) {
    PATH = path;
    empty_entry_maker = s;
    entry_len = entryLen;
