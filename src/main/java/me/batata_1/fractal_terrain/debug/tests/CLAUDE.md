...
| `GlobalRiverTest.java`        | Loads models, builds a `GlobalRiverProvider`, dumps every intermediate stage to PNG for a few tiles                                          | `globalRiverTest`        | Visual/manual global riverUnit network check                   |
| `LocalRiverTest.java`         | Loads models, builds `GlobalRiverProvider`/`LocalRiverProvider` directly, dumps flow/mask/carve/channel PNGs, checks bed monotonicity, dumps the unit tree | `localRiverTest`         | Visual/manual local riverUnit network check                    |
