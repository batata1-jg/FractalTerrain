...
| `MeandersTest.java`           | Manual assertion suite for `Meanders` graph ops (split/merge/collision capture/endpoint alignment) plus a relaxation scenario with PNG dumps | `meandersTest`           | Visual/manual meander relaxation + graph-invariant check   |
