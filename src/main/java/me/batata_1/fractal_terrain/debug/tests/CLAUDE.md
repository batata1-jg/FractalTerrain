...
| `LocalRiverTest.java`         | Loads models, builds `GlobalRiverProvider`/`LocalRiverProvider` directly, dumps flow/mask/carve/channel PNGs, checks bed monotonicity, dumps the primitive tree | `localRiverTest`         | Visual/manual local riverPrimitive network check                    |
...
| `CaptureSelectionTest.java`   | Manual reproduction of `MeandersGoldenTest`'s stream-capture fixtures; prints facts and a pass/fail summary rather than asserting, so `RiverNetwork.manageCollisions` can be observed without a compiling JUnit suite | `captureSelectionTest`   | Checking stream-capture selection without the JUnit suite  |
