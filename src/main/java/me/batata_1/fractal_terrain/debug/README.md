...
directly (`ModConfig`, `ChannelElevationAssigner`, `LocalDrainageTracer`, `LocalRiverProvider`,
`ImmutableRTree`, `PipelineModels`, most `debug/tests/` harnesses including `GlobalRiverTest`,
`LocalRiverTest`, `MeandersTest`, and `SpatialIndexBenchmark`, …) as go through the facade. The migration
