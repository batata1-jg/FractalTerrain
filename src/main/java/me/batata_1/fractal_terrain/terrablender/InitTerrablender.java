...
public class InitTerrablender {
    ...
    public static void initializeBlenderBiomes(DynamicRegistryManager registryAccess, RegistryKey<DimensionOptions> levelResourceKey, ChunkGeneratorSettings settings, FractalTerrainBiomeSource biomeSource, long seed) {
        ...
        Regions.get(regionType).forEach((region) -> region.addBiomes(biomeRegistry, (pair) -> builder.add(biomeRegistry.entryOf(pair.getSecond()))));
        ((IExtendedBiomeSource)biomeSource).appendDeferredBiomesList(builder.build());
        TerraBlender.LOGGER.info(String.format("Initialized TerraBlender biomes for level stem %s", levelResourceKey.getValue()));
