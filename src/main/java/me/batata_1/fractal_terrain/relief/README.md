...
high-frequency channel. `hydrology.RiverProvider` decodes the same residual separately to trace and carve
rivers; it also publishes its carved elevation as a `hydrology_relief` store, but `ReliefProvider` decodes
elevation channel 0 itself rather than reading that store, so the two providers still never share an
elevation buffer instance.
...
channels, `RiverProvider` to trace and carve off its own copy — but neither provider's output feeds
