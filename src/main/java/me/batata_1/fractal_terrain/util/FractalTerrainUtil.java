package me.batata_1.fractal_terrain.util;
...
public class FractalTerrainUtil {
    ...
    public static String giveNameToTile(Pair<Integer, Integer> xz) {
        return NorP(xz.getFirst()) + Math.abs(xz.getFirst()) + "-" + NorP(xz.getSecond()) + Math.abs(xz.getSecond());
    }
    ...
    public static Pair<Integer, Integer> interpretTileName(String s) {
        ...
        if (s.length() - 4 < 0 || m + 2 == s.length()) {
            LOGGER.error(
                    "invalid int tiles dir (wrong format): {} , s.len-4 = {} , m+1 = {}", s, s.length() - 4, m + 2);
            return null;
        }
