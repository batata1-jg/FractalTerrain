...
| `RiverInfluenceCarve.java`    | The stateless carve: `computeRiverGrid` (the LUT-backed lattice merge both the padded-tile shell and the per-chunk bed run), `carveRiverInfluenceGrid` wrapping it for the shell, and the thread-local `GridBuffers` each call site sizes | Carving the valley shell or the bed, the merge law, buffer sizing, adding a call site |
...
| `RadialProfile.java`          | The three radial shape laws: the confluence's parabolic bowl, the source's cone, and the abandoned river's shallower (0.4x) bowl; the radial twin of `RosgenProfile` | Adding/tuning a radially-carved feature's shape, changing the bowl or cone law |
| `InfluenceCarver.java`        | The enum dispatch (`ROSGEN`/`RADIAL`/`NONE`) that replaced the shell pass's old `instanceof` dispatch; which shell-carve algorithm a primitive uses | Adding a feature type's shell-carve behaviour, changing the shell dispatch |
