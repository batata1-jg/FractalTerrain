...
| `HydrologyProfileInprinter.java` | `computeRiverGrid`, the LUT-backed lattice carve both the shell (`carveRiverInfluence`) and the bed (via `PopulateNoiseStep`, fed by the chunk-level prefetch `prefetchChunk`) call; also hosts `sampleNearestChannel`, which has no `src/main` caller | Carving the valley shell or the bed, chunk-time prefetch, checking `sampleNearestChannel`'s live status |
