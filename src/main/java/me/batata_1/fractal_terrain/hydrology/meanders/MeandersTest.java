...
public class MeandersTest {
    ...
    private static Channel getDefaultChannel() {
        ...
        for (int i = 0; i < N; i++) {
            pts.add(new double[] {100.0 + i * 50.0, 1500.0 + 50.0 * Math.sin(2.0 * Math.PI * i / (N - 1))});
        }
    ...
    private static void testMeanders() {
        ...
        for (int i = 0; i < N; i++) {
            pts2.add(new double[] {10.0 + i * 5.0, (256) + 5.0 * Math.sin(2.0 * Math.PI * i / (N - 1))});
        }
        ...
        if (Math.abs(result.get(0)[0] - x0) > eps || Math.abs(result.get(0)[1] - z0) > eps)
            throw new AssertionError(String.format(
                    "Start endpoint moved: (%.6f,%.6f) → (%.6f,%.6f)", x0, z0, result.get(0)[0], result.get(0)[1]));
        if (Math.abs(result.get(n - 1)[0] - xEnd) > eps || Math.abs(result.get(n - 1)[1] - zEnd) > eps)
            throw new AssertionError(String.format(
                    "End endpoint moved: (%.6f,%.6f) → (%.6f,%.6f)",
                    xEnd, zEnd, result.get(n - 1)[0], result.get(n - 1)[1]));
        ...
        for (int i = 1; i < n; i++) {
            ...
            if (d < 25.0 || d > 100.0)
                throw new AssertionError(String.format("Spacing %.3f out of [25, 100] at i=%d", d, i));
        }

        // 4. Sinuosity growth
        if (sAfter <= sBefore)
            throw new AssertionError(String.format("Sinuosity did not grow: before=%.4f after=%.4f", sBefore, sAfter));
