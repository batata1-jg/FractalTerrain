...
| `RadialPrimitive.java`          | The interface the carve's radial pass dispatches on: disc extents, rim elevation, radial profile | Adding a radially-carved feature, the confluence/source/abandoned-river dispatch, extending the radial pass |
| `RosgenCarvedPrimitive.java`    | The interface `RiverPrimitive` and `OxbowLakePrimitive` both implement: flow tangent, width, curvature, Rosgen type — what `InfluenceCarver.ROSGEN` dispatches on | Adding a rectangle/tangent-carved feature, the river/oxbow shell dispatch |
...
| `OxbowLakePrimitive.java`       | A cutoff meander recorded as standing water; carves the same rotated-rectangle Rosgen cross-section as a live river | Oxbow recording, carve behaviour, `LAKE_BED` zone behaviour |
