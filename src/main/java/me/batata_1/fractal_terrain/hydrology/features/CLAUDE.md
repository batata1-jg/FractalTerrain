...
| `RadialPrimitive.java`          | The interface a disc-carved feature implements: extents, rim elevation, radial profile, and the shared `carveInfluence` no-op plus `carveBed`/`tabulateBedLut` defaults every implementer reuses | Adding a radially-carved feature, the confluence/source/abandoned-river dispatch, extending the disc carve |
| `RosgenCarvedPrimitive.java`    | The interface `RiverPrimitive` and `OxbowLakePrimitive` both implement: flow tangent, width, curvature, Rosgen type, and the shared `carveInfluence` default wired to the rectangle shell carve | Adding a rectangle/tangent-carved feature, the river/oxbow shell dispatch |
...
| `OxbowLakePrimitive.java`       | A cutoff meander recorded as standing water; shell-carves the same rotated-rectangle Rosgen cross-section as a live river, but its bed carve is not implemented (`carveBed`/`tabulateBedLut` throw) | Oxbow recording, shell carve behaviour, `LAKE_BED` zone behaviour |
| `AbandonedRiverPrimitive.java`  | A channel pruned by the collision pass; carves a radial disc, like a confluence pool left to silt in | Stream-capture leftovers, abandoned-channel carve behaviour |
