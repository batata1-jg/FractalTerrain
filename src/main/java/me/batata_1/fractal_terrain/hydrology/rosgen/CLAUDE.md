...
| ---------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| `README.md`                  | Key ordering rationale, measured-vs-prescribed inputs, calibration state    | Changing a threshold, diagnosing wrong stream types, before re-baselining |
| `RosgenKey.java`             | The ordered decision key. Pure, no raster or graph                          | Changing type boundaries, understanding why an ordering test fires |
...
| `ReachRosgenClassifier.java` | Reach segmentation + downstream-first graph walk; implements `ChannelTyper` | Reach length, classification ordering                              |
