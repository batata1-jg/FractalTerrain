...
import static me.batata_1.fractal_terrain.hydrology.PipelinePreprocessing.*;
...
final class LocalDrainageTracer {
    ...
    static void traceLocalNetwork(
            int[] drainage, float[] elev, RiverNetwork network , @Nullable Map<Integer,Double> boundaryElev, @Nullable LocalRiverProvider.Stages stages) {
        ...
        for(int cell = 0; cell < cellCount; cell++) {
            ...
            final int next = neighborIndex(cell,direction,PADDED);
            if(next == -1) continue;
        ...
        sourceQueue.clear();
        for (int cell = 0; cell < cellCount; cell++) {
            if (inDegree[cell] == 0) sourceQueue.add(cell);
        }
        ...
        final ImmutableRTree<GlobalRiverUnit> globalRiversPosition = new ImmutableRTree<>(getGlobalUnits(net),new GlobalRiverUnit(new double[]{0,0},0,-1));
        ...
        while (!sourceQueue.isEmpty()) {
            ...
            if(flow[current] >= FLOW_THRESHOLD) {
                ...
                if(nodeIndex[current] == -1) nodeIndex[current] = net.addNode(
                ...
                if(nodeIndex[next] == -1) {
                    final double[] nextNodePos = new double[]{Math.floorDiv(next,PADDED) + 0.5 , (next % PADDED) + 0.5};
