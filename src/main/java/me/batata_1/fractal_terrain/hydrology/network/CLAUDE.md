...
| `RiverNetwork.java`   | The graph: channels, nodes, the view seam, stream capture, `collectPrimitives` and its `collectExtendedPrimitives` sibling (emits `ExtendedRiverPrimitive` for the river family, carrying `channelId`/knot-index provenance) | Graph topology, `update` id rules, collision handling, emitting primitives    |
